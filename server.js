...
function gateKeeper(req, res, next) {
  ...
  const parsedHeader = queryString.parse(header,`userName=${header.user}&pass=${header.pass}`);
  console.log(header);
  console.log(parsedHeader);
  
  for (let i = 0; i<USERS.length; i++) {
    if (USERS[i].userName === parsedHeader.user && USERS[i].password == parsedHeader.pass) {
      req.user = USERS[i];
...
app.get("/api/users/me",gateKeeper);
